...
description: >
  Agent that handles ALM operations for Copilot Studio agents. Clones, pushes and pulls agent content between local YAML files and the cloud, publishes agents to make drafts live, lists environments and agents, and shows pending changes. Use for sync, deploy, publish, and lifecycle tasks.
skills:
  - int-project-context
  - manage-agent
  - clone-agent
